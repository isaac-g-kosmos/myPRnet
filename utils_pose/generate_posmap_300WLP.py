...
def run_posmap_300W_LP(
    ...
    cropped_image = skimage.transform.warp(
        image, tform.inverse, output_shape=(image_h, image_w)
    )
    ...
    uv_position_map = mesh.render.render_colors(
        uv_coords, bfm.full_triangles, position, uv_h, uv_w, c=3
    )
    ...
    io.imsave(
        "{}/{}/{}".format(save_folder, idx, "original.jpg"),
        np.squeeze(cropped_image),
    )
    np.save(
        "{}/{}/{}".format(save_folder, idx, image_name.replace("jpg", "npy")),
        uv_position_map,
    )
    io.imsave(
        "{}/{}/{}".format(save_folder, idx, "uv_posmap.jpg"),
        (uv_position_map) / max(image_h, image_w),
    )  # only for show
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_dir", help="specify input origin mat & image directory."
    )
    args = parser.parse_args()

    generate_batch_sample(save_folder=args.save_dir, input_dir=args.input_dir)
