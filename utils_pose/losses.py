...
def _ssim(
    img1, img2, window_size=11, window=None, val_range=2, size_average=True
):
...
def dfl_ssim(img1, img2, mask, window_size=11, val_range=1, gauss="original"):
    ...
    den1 = F.conv2d(
        img1 * img1 + img2 * img2, window, padding=padd, groups=channel
    )
...
class SSIM(torch.nn.Module):
    def __init__(self, mask_path, window_size=11, alpha=0.8, gauss="original"):
        ...
        if os.path.exists(mask_path):
            ...
            self.mask = (
                torch.from_numpy(preprocess(self.mask)).float().to("cuda")
            )
        else:
