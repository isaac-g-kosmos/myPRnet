...
"""
    ...
    @source: YadiraF/PRNet/utils_pose/cv_plot.py
"""
import numpy as np
import cv2
...
def plot_kpt(image, kpt):
    ...
    for i in range(kpt.shape[0]):
        ...
        ed = kpt[i + 1, :2]
        image = cv2.line(
            image, (st[0], st[1]), (ed[0], ed[1]), (255, 255, 255), 1
        )
    return image
...
def plot_pose_box(image, P, kpt, color=(0, 255, 0), line_width=2):
    ...
    point_3d_homo = np.hstack(
        (point_3d, np.ones([point_3d.shape[0], 1]))
    )  # n x 4
    point_2d = point_3d_homo.dot(P.T)[:, :2]
    point_2d[:, :2] = (
        point_2d[:, :2]
        - np.mean(point_2d[:4, :2], 0)
        + np.mean(kpt[:27, :2], 0)
    )
