...
class PRN:
    ...
    def __init__(self, is_dlib=False, is_opencv=False, prefix="."):
        ...
        prn_path = os.path.join(
            prefix, "Data/net-data/256_256_resfcn256_weight"
        )
        ...
        self.uv_kpt_ind = np.loadtxt(
            prefix + "/Data/uv-data/uv_kpt_ind.txt"
        ).astype(
            np.int32
        )  # 2 x 68 get kpt
        self.face_ind = np.loadtxt(
            prefix + "/Data/uv-data/face_ind.txt"
        ).astype(
            np.int32
        )  # get valid vertices in the pos map
        self.triangles = np.loadtxt(
            prefix + "/Data/uv-data/triangles.txt"
        ).astype(
            np.int32
        )  # ntri x 3
    ...
    def process(
        self, input, image_info=None, FaceRect_name_full=None, image_shape=None
    ):
        ...
        if image_info is not None:
            ...
            d = detected_faces[
                0
            ].rect  ## only use the first detected face (assume that each input image only contains one face)
            left = d.left()
            right = d.right()
            top = d.top()
            bottom = d.bottom()
